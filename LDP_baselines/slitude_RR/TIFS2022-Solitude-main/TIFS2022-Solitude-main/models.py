...
from torch_sparse import matmul, sum as sparsesum
...
class NodeClassifier(torch.nn.Module):
    def __init__(self,
                 input_dim,
                 num_classes,
                 model:                 dict(help='backbone GNN model', choices=['gcn', 'sage', 'gat']) = 'gcn',
    ...
    def cross_entropy_loss(p_y, y, weighted=False):
        ...
        y_onehot = F.one_hot(y.argmax(dim=1), num_classes=p_y.shape[1])
        # num_classes = p_y.shape[1]
        # y_onehot = torch.eye(num_classes)[int(y)]
