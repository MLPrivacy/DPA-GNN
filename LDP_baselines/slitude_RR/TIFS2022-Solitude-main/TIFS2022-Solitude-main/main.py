...
from transforms import FeaturePerturbation, EdgePerturbation,LabelPerturbation
        ...
        for all_eps in [10]:    #2,4,6,8,10
            for y_eps in [1]:    #1,1.5,2,2.5
                for eps_rate in [0.05]:  #0.05,0.2
                    ...
                    regular_list = [run(args) for _ in range(10)]
                    average = statistics.mean(regular_list)
                    std_dev = statistics.pstdev(regular_list)
                    print("Results for all_eps={}, eps_rate={}, x_eps={}, y_eps={}, e_eps={}".format(all_eps, eps_rate, x_eps, y_eps, e_eps), "blue")
                    print("Results:", round(average, 1), '+-', round(std_dev, 2))
                    with open('output.txt', 'a') as f:
                        print("Results for all_eps={}, eps_rate={}, x_eps={}, y_eps={}, e_eps={}".format(all_eps, eps_rate, x_eps, y_eps, e_eps),file=f)
                        print("Results:",  round(average, 1), '+-', round(std_dev, 2), file=f)
