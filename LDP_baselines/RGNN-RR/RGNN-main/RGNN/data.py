...
def load_dataset(dataset, val_ratio=.25, test_ratio=.25, cols_to_group=0,e_eps=0):
    ...
    if dataset in ['cora', 'citeseer', 'pubmed']:
        ...
        data = data[0]
    elif dataset in ['facebook', 'deezer', 'github', 'wikipedia', 'lastfm']:
        ...
        data_file_root = Path('./datasets') / 'Amazon_Computers'
        data = Amazon(root=data_file_root, name='Computers')
        data = data[0]
        ...
        data = Reddit(root=data_file_root)
        data = data[0]
        data = filter_class_by_count(data, remove_unlabeled=True)
        data = remove_isolated_nodes(data)
    ...
    data.y = torch.clamp(data.y, min=0, max=data.num_classes - 1).long()
