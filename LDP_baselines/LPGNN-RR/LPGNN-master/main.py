...
def run(args):
    ...
    for version in progbar:
        ...
        try:
            ...
            progbar.set_postfix({'last_test_acc': test_acc[-1], 'avg_test_acc': np.mean(test_acc),'std': np.std(test_acc)})
            if len(test_acc) == args.repeats:
                with open('out.txt', 'a') as f:
                    # print({'ex':args.x_eps,'ey':args.y_eps,'ee':args.e_eps,'dataset':args.dataset,'kx':args.x_steps,'ky':args.y_steps, 'avg_test_acc': round(np.mean(test_acc),1),'+-':round(np.std(test_acc),2)},file=f)
                    print(' | ex:', args.x_eps, ' | ey:',args.y_eps, ' | ee:', args.e_eps, ' | dataset:', args.dataset,
                           ' | kx:',args.x_steps, ' | ky:',args.y_steps, ' | avg_test_acc:',round(np.mean(test_acc), 1), '+-',
                          round(np.std(test_acc), 2), file = f)
                    print('---------------------------------------------------------------------------------',file=f)
